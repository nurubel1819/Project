...
void intro_screen() {
   ...
   int i;
   int width1 = textwidth("WELCOME TO");
   int height1 = textwidth("WELCOME TO");
   int maxx = getmaxx();
   int maxy = getmaxy();
   for (i = 0; i < 15; i++)
   {
      setfillstyle(i % 8, i);
      bar(i * 12, i * 12, maxx - i * 12, maxy - i * 12);
      delay(500);
   }
   ...
   delay(500);
	...
	outtextxy(maxx / 2 - width1 / 2 - 125, maxy / 2 - height1 / 2 - 60, "WELCOME TO");
	delay(2000);
	setcolor(YELLOW);
	settextstyle(10, HORIZ_DIR, 6);
	int width2 = textwidth("SNAKE GAME");
	int height2 = textheight("SNAKE GAME");
	outtextxy(maxx / 2 - width2 / 2, maxy / 2 - height2 / 2 + 15, "SNAKE GAME");
	delay(2000);
...
void howtoplay() // The tutorial screen
{
   ...
   outtextxy(maxx / 2 - mid_x / 2 + 20, maxy / 2 - mid_y / 2 - 250, "INSTRUCTIONS");
   ...
   outtextxy(177, 485, "as either one of these will make you die.");
   setcolor(3);
...
int main()
{
    MainWindow = initwindow(1024, 768, "Snake Game", 240, 10, false, false);
    intro_screen();
